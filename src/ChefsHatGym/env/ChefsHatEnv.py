...
class ChefsHatEnv(gym.Env):
    ...
    def startNewGame(self):
        ...
        if not self.logDirectory=="":
            ...
            self.experimentManager = ExperimentManager.ExperimentManager(self.logDirectory, experimentName, verbose=self.verbose, saveLog=self.saveLog)
    ...
    def startNewmatch(self):
        ...
        self.cards = flat([[v for _ in range(v)] for v in range(1, self.maxCardNumber+1)]) + [self.maxCardNumber+1 for _ in range(2)]
    ...
    def step(self, action):
        ...
        if self.isActionAllowed(thisPlayer, action, possibleActions):  # if the player can make the action, do it.
            ...
            if self.saveDataset:
                self.experimentManager.dataSetManager.doActionAction(self.matches, thisPlayer, self.rounds, actionComplete, self.board, 0, 0, self.playersHand, self.currentRoles, self.score, self.lastActionPlayers, action, 0, 0, validAction)
    ...
    def makePizza(self):
        ...
        if ((numberFinished >= len(playersInGame) -1 ) and (numberOfActions == len(playersInGame)   )):
            ...
            if self.saveDataset:
                #     # print ("Pizza!")
                self.experimentManager.dataSetManager.doActionPizzaReady(self.rounds, self.board, self.playersHand, self.currentRoles, self.score, self.lastActionPlayers, self.matches)
    ...
    def dealCards(self):
        ...
        for playerNumber in range(len(self.playersHand)):
            self.playersHand[playerNumber] = sorted(self.cards[playerNumber * self.numberOfCardsPerPlayer:playerNumber * self.numberOfCardsPerPlayer + self.numberOfCardsPerPlayer])
